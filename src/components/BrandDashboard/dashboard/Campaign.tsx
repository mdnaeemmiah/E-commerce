...
export default function Campaign() {
  ...
  useEffect(() => {
    ...
    const fetchCampaigns = async () => {
      try {
        const response = await baseApi.get(ENDPOINTS.allCampaigns, { headers });

        // Check if campaigns exist in the response data
  ...
  if (campaigns.length === 0) {
    return <p>No campaigns available.</p>; // Display if no campaigns are available
  }
          ...
          <div key={i} className="w-full border border-gray-200 rounded-xl p-5 bg-white">
            ...
            <div className="w-full h-2 bg-gray-200 rounded-full mt-4 overflow-hidden">
              <div
                className="h-full bg-purple-500 rounded-full"
                style={{
                  width: `${Math.round((item.spent_amount / item.budget) * 100)}%`,
                }}
              ></div>
            </div>
          </div>
