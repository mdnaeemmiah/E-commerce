...
//     try {
//       const response = await baseApi.get(ENDPOINTS.getTransactionHistory, {
//         headers: {
//           Authorization: `Bearer ${token}`,
//           "Content-Type": "application/json",
//         },
...
//       setWalletPayment(response.data); // Store wallet payment in state
//     } catch (error) {
//       console.error("Error fetching wallet TransactionHistory:", error);
//       toast.error("Failed to fetch wallet TransactionHistory. Please try again.");
//     }
//   };
...
//         {walletPayment && walletPayment.length > 0 ? (
//           walletPayment.map((transaction, i) => (
//             <div
//               key={i}
//               className="flex justify-between items-center py-3 text-gray-700 border-b border-gray-200"
//             >
//               <div>
//                 <p>{transaction.type}</p> {/* Assuming `type` is part of your data */}
...
//               <span
//                 className={`${
//                   transaction.status === "Completed" ? "bg-[#15983B]" : "bg-[#FF5733]"
//                 } text-[14px] text-white rounded-xl p-1`}
//               >
//                 {transaction.status}
//               </span>
//             </div>
...
export default function Wallet() {
  ...
  // Consolidate API calls in a single useEffect
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch Rewards
        const rewardsResponse = await baseApi.get(ENDPOINTS.getRewards, {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
          },
        ...
        // Fetch Wallet Payment Data
        const walletPaymentResponse = await baseApi.get(
        ...
        setWalletPayment(walletPaymentResponse.data);

        // Fetch Transaction History
        const transactionHistoryResponse = await baseApi.get(
        ...
        setTransactionHistory(transactionHistoryResponse.data); // Store transaction history separately
      } catch (error) {
